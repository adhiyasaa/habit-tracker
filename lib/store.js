...
import { getWeekStartDate, formatDate } from "./constants";

// Add a new activity
export async function addActivity({ userId, type, customName, notes, date, mood, difficulty }) {
  try {
    // New point system: gym and lari = 10 points, others = 3 points
    const points = (type === "gym" || type === "lari") ? 10 : 3;
...
export async function getWeeklyStats(userId, refDate = new Date()) {
  try {
    ...
    // Robust point calculation helper
    const getPoints = (a) => {
      if (a.points !== undefined && a.points !== null) return a.points;
      return (a.type === "gym" || a.type === "lari") ? 10 : 3;
    };
    ...
    // Fetch all activities ever to calculate Cumulative/Lifetime Points
    ...
    const allTimePoints = allErr
      ? totalPoints
      : allActivities.reduce((sum, a) => sum + getPoints(a), 0);

    // Fetch latest affirmation written by this user (type === "apresiasi")
    ...
    const latestAffirmation = affData?.[0] || null;
